...
class HDRISettings(PropertyGroup):
    ...
    def update_hdri_preview(self, context):
        ...
        if render_engine == 'VRAY_RENDER_RT':
            ...
            # Check if Octane HDRI system is properly initialized
            # We need all three key nodes to consider it initialized
            rgb_node = None
            transform_node = None
            tex_env_node = None
            ...
            # Only proceed if the system is fully initialized
            # This prevents automatic node creation when engine is set to Octane
            if not (rgb_node and transform_node and tex_env_node):
                print(f"Octane HDRI system not initialized - skipping HDRI update")
                return
            ...
            if rgb_node:
                try:
                    ...
                    if current_image:
                        # Get the original path properly
                        current_path = original_paths.get(current_image.name)
                        if not current_path and hasattr(rgb_node, 'a_filename') and rgb_node.a_filename:
                            # Try to get from a_filename
                            current_path = original_paths.get(os.path.basename(rgb_node.a_filename))
                        if not current_path and hasattr(rgb_node, 'a_filename'):
                            # Fallback to a_filename directly
                            current_path = rgb_node.a_filename

                    # Check if we're trying to load the same HDRI that's already loaded
                    # Compare original paths, not proxy paths
                    # This happens when preview is updated from reset button
                    if current_path:
                        # Get the original path from current_path if it's a proxy
                        current_original = original_paths.get(os.path.basename(current_path), current_path)
                        # Also handle if current_path itself is in original_paths by name
                        if os.path.basename(current_original) in original_paths:
                            current_original = original_paths[os.path.basename(current_original)]

                        # Compare original paths
                        if current_original == filepath:
                            print(f"Octane: Already have {filepath} loaded, skipping update")
                            return

                    # Only update previous_hdri_path if we're actually changing HDRIs
                    if current_image and current_path:
                        self.previous_hdri_path = original_paths.get(os.path.basename(current_path), current_path)
                    ...
                    # CRITICAL: Store comprehensive path mappings for toggling to work
                    # Store by image name
                    original_paths[new_image.name] = filepath
                    # Store by basename of loaded path (proxy or original)
                    original_paths[os.path.basename(target_path)] = filepath
                    # Store by basename of original filepath
                    original_paths[os.path.basename(filepath)] = filepath
                    print(f"Octane: Stored path mappings for {new_image.name} -> {filepath}")
...
def initialize_proxy_settings_from_preferences(scene):
    ...
    try:
        ...
        # Always sync with preferences on load - users expect their preference settings
