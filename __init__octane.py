...
class HDRI_PT_controls(Panel):
    ...
    def draw(self, context):
        ...
        # Display render engine in header
        header_row = main_column.row(align=True)
        header_row.label(text="Octane Build", icon='BLENDER')
        ...
        if has_active_hdri(context):  # Only show settings if an HDRI is active
            ...
            if hdri_settings.show_rotation:
                ...
                meta_row.prop(hdri_settings, "show_metadata",
                    ...
                    icon_only=True,
                    emboss=False)
                if hdri_settings.show_metadata:
                    meta_box = rotation_box.box()
                    meta_col = meta_box.column(align=True)
                    meta_col.scale_y = 0.9
                    # Get RGB Image node for metadata
                    rgb_node = None
                    for node in world.node_tree.nodes:
                        if node.bl_idname == 'OctaneRGBImage':
                            rgb_node = node
                            break
                    if rgb_node and rgb_node.a_filename:
                        filepath = rgb_node.a_filename
                        filename = os.path.basename(filepath)
                        ...
                        # Resolution (get from loaded image)
                        try:
                            img = bpy.data.images.get(filename) or bpy.data.images.load(filepath)
                            if img:
                                row = meta_col.row(align=True)
                                row.label(text="Resolution:", icon='TEXTURE')
                                row.label(text=f"{img.size[0]}x{img.size[1]}")
                        except:
                            pass
                        ...
                        try:
                            ...
                            size_str = convert_size(size_bytes)
                            row = meta_col.row(align=True)
                        ...
                        row.label(text=os.path.splitext(filename)[1].upper()[1:])                      
